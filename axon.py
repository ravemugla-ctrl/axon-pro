        ...
        if st.button(f"🚀 {target_company} Masasına Otur"):
            ...
            def ask_agent(prompt):
                try:
                    ...
                    return f"Ajan Hatası: {e}"

            with st.status(f"Ajanlar {target_company} ile gizli oturumda...", expanded=True) as status:
                with tabs[0]:
                    st.markdown("### 🏹 Satış Uzmanı Raporu")
                    st.write(ask_agent(f"{target_company} için {record_count} satırlık {data_category} verisini {total_valuation}$ fiyatla satacak profesyonel ve ikna edici bir teklif yaz. Tarz: {negotiation_style}"))
                with tabs[1]:
                    st.markdown("### ⚖️ Hukuk Müşaviri Taslağı")
                    st.write(ask_agent(f"{data_category} verisinin {target_company} firmasına mikro-lisanslanması için profesyonel bir gizlilik sözleşmesi taslağı hazırla."))
                with tabs[2]:
                    st.markdown("### 📉 Pazar Analisti Görüşü")
                    st.write(ask_agent(f"{data_category} verisinin 2026 pazarındaki değerini analiz et. {total_valuation}$ uygun mu?"))
